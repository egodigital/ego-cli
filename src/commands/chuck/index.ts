...
export class EgoCommand extends CommandBase {
    ...
    public async execute(context: CommandExecuteContext): Promise<void> {
        ...
        if (context.args['n'] || context.args['name']) {
            ...
            if (split.length === 2) {
                url += '?firstName='+split[0]+"&lastName="+split[1];
                url += "&escape=javascript";
            }else if(split.length === 1 && split[0].length > 1){
                url += '?firstName='+split[0]+"&lastName= ";
                url += "&escape=javascript";
            }else{
    ...
    public async showHelp(): Promise<void> {
        ...
        writeLine(`Example:     ego chuck --name Ole-Mustermann`);
        writeLine(`Example:     ego chuck -n Ole`);
        writeLine(`Example:     ego chuck -i 15`);
        writeLine(`Example:     ego chuck --id 15 -n Ole-Mustermann`);
    }

    /** @inheritdoc */
    public readonly syntax = '[options]';  // Syntax, that is shown in help screen

    private getJoke = function(url): Promise<any> {
